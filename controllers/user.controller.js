...
const handleViewAuditorium=async function(req,res){
  try{
    ...
    await Promise.all(list.map(async auditorium => {
        const outputPath = `/Users/shivakumar/Desktop/projects/web projects/auditorium_booking/public/images/outputs/${auditorium.bannerImage}`;
        try {
...
const handleBookAuditorium = async function(req,res){
  try{
    const imageurl = `/Users/shivakumar/Desktop/projects/web projects/auditorium_booking/public/images/${req.body.bannerImage}`;
    ...
    noOfAuditoriums=[];
    const statusDisplayText= anyOverlap.length === 0 ? `Booked Auditorium from ${startDate} ${startTiming} to ${endDate} ${endTiming} Successfully`: 'Auditorium Booked on that dates';
